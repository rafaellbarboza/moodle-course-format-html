...
YUI.add('moodle-format_grid_across-gridkeys', function (Y, NAME) {
...
M.format_grid_across = M.format_grid_across || {};
M.format_grid_across.gridkeys = M.format_grid_across.gridkeys || {};
M.format_grid_across.gridkeys = {
    init: function() {
        Y.on('esc', function (e) {
            e.preventDefault();
            Y.log("Esc pressed");
            Y.log("Selected section no: " + M.format_grid_across.selected_section_no);
            M.format_grid_across.icon_toggle(e);
        });
        Y.on('left', function (e) {
            e.preventDefault();
            Y.log("Left pressed");
            M.format_grid_across.arrow_left(e);
        });
        Y.on('right', function (e) {
            e.preventDefault();
            Y.log("Right pressed");
            M.format_grid_across.arrow_right(e);
        });
        /* Deferred functionality - see CONTRIB-3240...
        Y.on('enter', function (e) {
            //var ae = document.activeElement;
            if (M.format_grid_across.shadebox.shadebox_open == false) {
                e.preventDefault();
                Y.log("Enter pressed");
                Y.log("Selected section no: " + M.format_grid_across.selected_section_no);
                M.format_grid_across.icon_toggle(e);
